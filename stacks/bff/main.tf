##
## Stack: bff
## Deploys a BFF (Backend For Frontend) service on the shared ECS cluster.
## Registers a path-based listener rule on the public ALB.
##
...
# ── Read platform outputs from SSM (when ssm_prefix is provided) ─────────────
...
locals {
  ...
  public_alb_listener_arn = local.use_ssm ? data.aws_ssm_parameter.public_alb_listener_arn[0].value : var.public_alb_listener_arn
...
data "aws_ssm_parameter" "public_alb_listener_arn" {
  count = local.use_ssm ? 1 : 0
  name  = "${var.ssm_prefix}/public_alb_listener_arn"
}
...
module "task_role" {
  ...
  role_description = "Task role for BFF service ${var.service_name}"
...
resource "aws_lb_listener_rule" "this" {
  listener_arn = local.public_alb_listener_arn
